...
public class HybridExecuteTest  {   
    ...
    public void runManager(String browser) throws Exception {    	    	 
        try {        	
            ...
            List<String> arrGroupList = InitScript.readGroupData(Global.gstrControlFilesDir + "COD R13 Cloud Erp Control File.xlsx",strGroupQuery);            
            for(String strGroupName : arrGroupList) {            	
            	String strGroupSheetQuery = "Select * from "+ strGroupName +" where Run='Y'";            
                List<String> arrTCList = InitScript.readGroupData(Global.gstrControlFilesDir + "COD R13 Cloud Erp Control File.xlsx",strGroupSheetQuery);                         	
                for(String strTCID : arrTCList) {                 	
                	strGroupSheetQuery = "Select * from "+ strGroupName +" where ID='"+strTCID+"'";            
                	String strBatchName = InitScript.readGroupSheetData(Global.gstrControlFilesDir + "COD R13 Cloud Erp Control File.xlsx",strGroupSheetQuery);
                	//Driver initialization
                	Utility util = new Utility(browser);
                	//Read Batch
                	String strBatchQuery = "Select * from BatchSheet where BatchTestFile='"+ strBatchName +"'";
                	List<String> arrBatchList = InitScript.readBatchData(Global.gstrBatchFilesDir + "COD R13 Cloud Erp BatchSheet.xlsx",strBatchQuery);   
